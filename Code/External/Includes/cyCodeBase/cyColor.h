...
class cyColor
{
	friend cyColor operator+(const float v, const cyColor& c)
	{
		return c + v;
...
public:
	...
	cyColor& Set(float _r, float _g, float _b)
	{
		r = _r;
		g = _g;
		b = _b;
	...
	void ClampMinMax(float min = 0, float max = 1)
	{
		ClampMin(min);
		ClampMax(max);
	}

	void ClampMin(float n = 0)
	{
		if (r < n)
			r = n;
		if (g < n)
	...
	cyColor operator-() const
	{
		return cyColor(-r, -g, -b);
	}
	...
	cyColor operator+(const cyColor& c) const
	{
		return cyColor(r + c.r, g + c.g, b + c.b);
	}
	...
	cyColor operator+(float n) const
	{
		return cyColor(r + n, g + n, b + n);
	}
	...
	///@name Test operators
	int operator==(const cyColor& c) const
	{
...
class cyColorA
{
	friend cyColorA operator+(const float v, const cyColorA& c)
	{
		return c + v;
	} ///< Addition with a constant
	friend cyColorA operator-(const float v, const cyColorA& c)
...
public:
	...
	cyColorA& Set(float _r, float _g, float _b, float _a)
	{
		r = _r;
		g = _g;
		b = _b;
	...
	///@name Unary operators
	cyColorA operator-() const
	{
		return cyColorA(-r, -g, -b, -a);
	}
	...
	///@name Binary operators
	cyColorA operator+(const cyColorA& c) const
	{
		return cyColorA(r + c.r, g + c.g, b + c.b, a + c.a);
	}
	...
	cyColorA operator+(float n) const
	{
		return cyColorA(r + n, g + n, b + n, a);
	}
	...
	///@name Assignment operators
	cyColorA& operator+=(const cyColorA& c)
	{
		r += c.r;
		g += c.g;
		b += c.b;
	...
	///@name Test operators
	int operator==(const cyColorA& c) const
	{
...
public:
	...
	///@name Conversion Methods
	cyColor ToColor() const
	{
		return cyColor(r / 255.0f, g / 255.0f, b / 255.0f);
	}
	...
	cyColor24& Set(unsigned char _r, unsigned char _g, unsigned char _b)
	{
		r = _r;
		g = _g;
		b = _b;
	...
	cyColor24& Set(const unsigned char* v)
	{
		r = v[0];
		g = v[1];
		b = v[2];
...
public:
	...
	cyColorA ToColorA() const
	{
		return cyColorA(r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f);
	}

	///@name Set & Get value functions
	cyColor32& Black(unsigned char _a = 255)
	{
		r = 0;
		g = 0;
		b = 0;
	...
	cyColor32& White(unsigned char _a = 255)
	{
		r = 255;
		g = 255;
		b = 255;
	...
	cyColor32& Set(unsigned char _r, unsigned char _g, unsigned char _b, unsigned char _a)
	{
		r = _r;
		g = _g;
		b = _b;
	...
	cyColor32& Set(const unsigned char* v)
	{
		r = v[0];
		g = v[1];
		b = v[2];
