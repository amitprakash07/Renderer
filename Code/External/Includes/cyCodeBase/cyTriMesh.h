...
public:
	...
	{
		bool result = boundMin.x != 0 && boundMin.y != 0 && boundMin.z != 0 && boundMax.x != 0 && boundMax.y != 0 && boundMax.z != 0;
		return result;
	}
	...
	{
		return Interpolate(faceID, v, f, bc);
	...
	{
		return Interpolate(faceID, vn, fn, bc);
	...
	{
		return Interpolate(faceID, vt, ft, bc);
	...
	{
		int result = static_cast<int>(mtlID > 0 ? mcfc[mtlID] - mcfc[mtlID - 1] : mcfc[0]);
		return result;
	} ///< Returns the number of faces associated with the given material ID.
	int GetMaterialFirstFace(int mtlID) const
	{
		int result  = static_cast<int>(mtlID > 0 ? mcfc[mtlID - 1] : 0);
		return result;
...
private:
	...
	static cyPoint3f Interpolate(int i, const cyPoint3f* v, const cyTriFace* f, const cyPoint3f& bc)
	{
			...
			for (int i = 2; i < rb; i++)
			{
				...
				else
				{
					...
					if (buffer[i] >= '0' && buffer[i] <= '9')
					{
						...
						switch (type)
						{
						case 0: f[rf].v[facevert] = index - 1;
							break;
						case 1: if (ft)
		...
		for (unsigned int mi = 0; mi < mtlFiles.size(); mi++)
		{
			...
			while (int rb = buffer.ReadLine(fp))
			{
				if (buffer.IsCommand("newmtl"))
				{
					...
					if (mtlID >= 0)
						strncpy(m[mtlID].name, mtlName, 256);
				}
				else if (mtlID >= 0)
				{
					if (buffer.IsCommand("Ka"))
						...
						buffer.Copy(m[mtlID].map_Ka.name, 256, 7);
					else if (buffer.IsCommand("map_Kd"))
						buffer.Copy(m[mtlID].map_Kd.name, 256, 7);
					else if (buffer.IsCommand("map_Ks"))
						buffer.Copy(m[mtlID].map_Ks.name, 256, 7);
				}
			}
			fclose(fp);
		}
