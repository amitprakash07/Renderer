...
class cyPoint2f
{
	friend cyPoint2f operator+(const float v, const cyPoint2f& pt)
	{
		return pt + v;
...
public:
	...
	cyPoint2f& Set(float _x, float _y)
	{
		x = _x;
		y = _y;
		return *this;
	} ///< Sets x and y coordinates as given
	cyPoint2f& Set(const float* v)
	{
		x = v[0];
		y = v[1];
		return *this;
	...
	{
		return cyPoint2f(x + pt.x, y + pt.y);
	}
	...
	cyPoint2f& operator+=(const cyPoint2f& pt)
	{
		x += pt.x;
		y += pt.y;
		return *this;
...
class cyPoint3f
{
	friend cyPoint3f operator+( const float v, const cyPoint3f &pt ) 
	{
		return pt+v;
	}		///< Addition with a constant
...
public:
	...
	float Length()const
	{
		float length = static_cast<float>(sqrt(LengthSquared()));
		return length;
	}
	...
	{
		cyPoint3f temp = cyPoint3f(-x, -y, -z);
		return temp;
	}
	...
	{
		cyPoint3f temp = cyPoint3f(x + pt.x, y + pt.y, z + pt.z);
		return temp;
	}
	...
	{
		cyPoint3f temp = cyPoint3f(x + n, y + n, z + n);
		return temp;
	}
	...
	{
		cyPoint3f temp = cyPoint3f(y*pt.z - z*pt.y, z*pt.x - x*pt.z, x*pt.y - y*pt.x);
		return temp;
	}	///< Cross product
	...
	{
		float dotProduct = x*pt.x + y*pt.y + z*pt.z;
		return dotProduct;
	}	///< Dot product
...
class cyPoint4f
{
	friend cyPoint4f operator+(const float v, const cyPoint4f& pt)
	{
		return pt + v;
	} ///< Addition with a constant
	friend cyPoint4f operator-(const float v, const cyPoint4f& pt)
...
public:
	...
	{
		return cyPoint4f(x + pt.x, y + pt.y, z + pt.z, w + pt.w);
	}
	...
	{
		return cyPoint4f(x + n, y + n, z + n, w + n);
	}
