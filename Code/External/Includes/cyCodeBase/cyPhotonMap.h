...
public:
	...
	public:
		void SetPower(const cyColor &c);
		void GetPower(cyColor &c) const { c = color.ToColor() * power; }
		void ScalePower(float scale) { power *= scale; }
		float GetMaxPower() const { return power; }
		void SetDirection(const cyPoint3f &dir);
		void GetDirection(cyPoint3f &dir) const;
	...
	void AddPhoton( const cyPoint3f &pos, const cyPoint3f &dir, const cyColor &power );
	...
	void EstimateIrradiance( cyColor &irrad, cyPoint3f &direction, float radius, const cyPoint3f &pos,
		const cyPoint3f *normal=NULL, float ellipticity=1, FilterType filterType=FILTER_TYPE_CONSTANT ) const;

	/// Returns the closest photon to the given position.
	/// If no photon is found within the radius, returns false.
	bool GetNearestPhoton( Photon &photon, float radius, const cyPoint3f &pos, const cyPoint3f *normal=NULL, float ellipticity=1 ) const;
...
private:
	...
	void BalanceSegment( std::vector<Photon> &balancedMap, const cyPoint3f &boxMin, const cyPoint3f &boxMax, unsigned int index, unsigned int start, unsigned int end );
...
inline void cyPhotonMap::Photon::SetPower(const cyColor &c)
{
	power = c.r;
...
inline void cyPhotonMap::Photon::SetDirection(const cyPoint3f &dir)
{
...
inline void cyPhotonMap::Photon::GetDirection(cyPoint3f &dir) const
{
...
inline void cyPhotonMap::AddPhoton( const cyPoint3f &pos, const cyPoint3f &dir, const cyColor &power )
{
...
inline void cyPhotonMap::PrepareForIrradianceEstimation()
{
	if ( photons.size() == 0 ) return;

	// compute bounding box
	cyPoint3f boxMin = photons[0].position;
	cyPoint3f boxMax = photons[0].position;
...
inline void cyPhotonMap::BalanceSegment( std::vector<Photon> &balancedMap, const cyPoint3f &boxMin, const cyPoint3f &boxMax, unsigned int index, unsigned int start, unsigned int end )
{
	// find median
	...
	// find splitting axis
	int axis = 2;
	cyPoint3f boxDif = boxMax - boxMin;
...
template <int maxPhotons>
inline void cyPhotonMap::EstimateIrradiance(cyColor &irrad, cyPoint3f &direction, float radius, const cyPoint3f &pos, const cyPoint3f *normal, float ellipticity, FilterType filterType) const
{
	irrad.Black();
	direction.Zero();
...
inline bool cyPhotonMap::GetNearestPhoton( cyPhotonMap::Photon &photon, float radius, const cyPoint3f &pos, const cyPoint3f *normal, float ellipticity ) const
{
	float found_dist2[2];
...
inline void cyPhotonMap::LocatePhotons( NearestPhotons &np, const int index ) const
{
	...
	cyPoint3f dif = p.position - np.pos;
